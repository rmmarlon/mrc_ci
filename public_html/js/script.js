 ...
 */
function visualizarFoto(arquivo,idCaminho) {
    ...
    // Lê a imagem upada arquivo
    oFReader.readAsDataURL(document.getElementById(arquivo).files[0]);

    // "Pinta" a imagem no elemento de imagem idCaminho
    oFReader.onload = function (oFREvent) {
        document.getElementById(idCaminho).src = oFREvent.target.result;
    };
};
function ajaxInit( ) {
	var xmlhttp;
...
$(document).ready(function(){
	...
	$(".cep").css("width", "9em");
	$(".cpf").css({"width": "9em",'text-align':'left'});
	$(".date").css("width", "6.15em");
	$(".phone").css("width", "8.15em");
	$(".time").css("width", "40px");
	...
	$(".pagination li a").live('click',function(e){
		var url = location.href.substr(27);
		var id = $(this).attr("id");
		e.preventDefault();
		$("body").load(url+"?pagina="+id);
...
$(document).ready(function() {
	...
	$('a[name=modal]').click(function(e) {
		...
		$(id).css({'width':'35%','color':'#000','font-family':'verdana', 'left':'31%'});
		$(id).slideDown().animate({top:'15%'},1000);
	});
	...
	$('a[name=modalB]').click(function(e) {
		...
		$(id).css({'width':'55%','color':'#000','font-family':'verdana', 'left':'20%'});
		$(id).slideDown().animate({top:'15%'},1000);
